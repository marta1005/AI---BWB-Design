...
def main() -> None:
    ...
    ffd_path = output_dir / "reference_semispan_ffd.xyz"
    summary_path = output_dir / "reference_semispan_ffd.json"
    figure_path = output_dir / "reference_semispan_ffd.png"

    case, dvgeo = create_reference_dvgeo_case(output_ffd_path=ffd_path, ffd_spec=FFDBoxSpec(), full_wing=False)
    write_dvgeo_baseline_outputs(dvgeo, case.pointset_name, output_dir, stem="reference_semispan_baseline")
    plot_ffd_with_surface(
        ffd_points=read_plot3d_ffd(ffd_path),
        upper_grid=case.upper_grid,
        lower_grid=case.lower_grid,
        out_path=figure_path,
        title="Reference semispan FFD loaded with DVGeometry",
    ...
    print(f"DVGeometry Tecplot written to: {output_dir / 'reference_semispan_baseline_ffd.dat'}")
    print(f"DVGeometry pointset written to: {output_dir / 'reference_semispan_baseline_surface_wing_surface.dat'}")
