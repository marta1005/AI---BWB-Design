...
def project_dependency_root() -> Path:
    ...
    # Keep CTA core independent from legacy package location.
    # We resolve dependencies by searching upward for a sibling "project" folder
    # containing both pyspline and pygeo sources.
    for parent in (package_dir, *package_dir.parents):
        candidate = parent / "project"
        if (candidate / "pyspline").is_dir() and (candidate / "pygeo").is_dir():
            return candidate
    # Fallback to the historical layout.
    return package_dir.parent / "project"
