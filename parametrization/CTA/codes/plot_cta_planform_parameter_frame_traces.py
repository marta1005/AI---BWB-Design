...
from parametrization.CTA.codes.animate_cta_planform_parameter_sweep import build_planform_sweep_samples


OUTPUT_PNG = SCRIPT_DIR.parent / "outputs" / "reference" / "cta_planform_parameter_frame_traces.png"
OUTPUT_CSV = SCRIPT_DIR.parent / "outputs" / "reference" / "cta_planform_parameter_frame_traces.csv"
...
def main() -> None:
    ...
    for ax, (name, label, units, lower, upper) in zip(axes_flat, specs):
        ...
        reference_value = _c4_absolute(reference) if name == "c4_absolute" else float(reference[ref_key])
        ...
        ax.axhline(reference_value, color="#475569", linewidth=1.0, linestyle=":", label="Reference")
