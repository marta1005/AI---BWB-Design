...
from parametrization.CTA.codes.animate_cta_planform_parameter_sweep import _public_sample_to_reference_design
from parametrization.bwb.planform import build_sectioned_bwb_planform


OUTPUT_DIR = SCRIPT_DIR.parent / "outputs" / "reference" / "planform_parameter_variations"
GRID_PNG = SCRIPT_DIR.parent / "outputs" / "reference" / "cta_planform_parameter_variations_grid.png"
...
def _sample_with_single_override(name: str, value: float) -> Dict[str, float]:
    ...
    sample = dict(space.reference_flat())
...
def _parameter_specs(bounds: Dict[str, Tuple[float, float]], reference: Dict[str, float]):
    return [
        {
            "name": "c1_root_chord",
            "title": "C0 variation",
            ...
            "reference": float(reference["c1_root_chord"]),
        ...
        },
        {
            ...
            "reference": float(reference["c2_c1_ratio"]),
        ...
        },
        {
            ...
            "reference": float(reference["c4_c3_ratio"]),
        ...
        },
        {
            ...
            "reference": float(reference["c4_c1_ratio"]),
        ...
        },
        {
            ...
            "reference": float(reference["s2_deg"]),
        ...
        },
        {
            ...
            "reference": float(reference["s3_deg"]),
        ...
        },
        {
            ...
            "reference": float(reference["b2_span_ratio"]),
        ...
        },
        {
            ...
            "reference": float(reference["span"]),
        ...
        },
        {
            ...
            "reference": float(reference["med_3_te_sweep_deg"]),
...
def _plot_variation_panel(
    ...
    levels = [
        ...
        ("Reference", float(spec["reference"]), "#0f172a"),
...
def main() -> None:
    ...
    bounds = space.bounds
    ref_config = to_cta_model_config(build_reference_design())
    reference_curves = _planform_curves(ref_config)
    ref_dense_span, ref_le, ref_te = reference_curves
    ...
    specs = _parameter_specs(bounds, reference)

    fig, axes = plt.subplots(3, 3, figsize=(17.5, 15.0), constrained_layout=True)
    axes_flat = axes.ravel()
    for ax, spec in zip(axes_flat, specs):
        _plot_variation_panel(ax, spec, reference_curves, y_sections, xlim, ylim)
    ...
    for spec in specs:
        fig_one, ax_one = plt.subplots(figsize=(10.4, 8.2), constrained_layout=True)
        _plot_variation_panel(ax_one, spec, reference_curves, y_sections, xlim, ylim)
        out_name = f"{spec['name']}_variation.png"
        fig_one.savefig(OUTPUT_DIR / out_name, dpi=200)
        plt.close(fig_one)

    print(f"CTA planform variation grid written to: {GRID_PNG}")
    print(f"CTA planform individual variation PNGs written to: {OUTPUT_DIR}")
