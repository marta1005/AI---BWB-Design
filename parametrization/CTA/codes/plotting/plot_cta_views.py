...
def glider_anchor_vertical_traces(anchor_y: np.ndarray):
    geo_path = CTA_DIR / "airfoils" / "bwb_glider.geo"
    if not geo_path.exists():
        return None
    ...
    for yy in np.asarray(anchor_y, dtype=float):
        raw_key = min(sections.keys(), key=lambda value: abs(value - float(yy)))
        if abs(float(raw_key) - float(yy)) > 5.0e-3:
            return None
        raw = np.asarray(sections[raw_key], dtype=float)
        le_idx = int(np.argmin(raw[:, 0]))
        upper_anchor.append(float(np.max(raw[:, 1])))
        lower_anchor.append(float(np.min(raw[:, 1])))
        le_anchor.append(float(raw[le_idx, 1]))
        te_upper_anchor.append(float(raw[0, 1]))
        te_lower_anchor.append(float(raw[-1, 1]))
        te_mid_anchor.append(0.5 * (float(raw[0, 1]) + float(raw[-1, 1])))

    return (
        np.asarray(upper_anchor, dtype=float),
        np.asarray(lower_anchor, dtype=float),
        np.asarray(le_anchor, dtype=float),
        np.asarray(te_upper_anchor, dtype=float),
        np.asarray(te_lower_anchor, dtype=float),
        np.asarray(te_mid_anchor, dtype=float),
    )
...
def annotate_layout_measurements(
    ...
    taper_ratio = float(cta_view["c4_c3_ratio"])
...
def annotate_public_nomenclature_scheme(
    ...
    # Mark the public profile/chord stations shown in the CTA case
    ...
    public_chord_labels = {
        ...
        3: ("Transition taper ratio", float(cta_view["c4_c3_ratio"]), 1.18),
...
def draw_profiles(
    ...
    for yy, chord in zip(y_sections, chords):
        ...
        zu = yu * float(chord)
        zl = yl * float(chord)
        twist_deg = float(prepared.spanwise_laws.twist_deg(float(yy)))
        twist_rad = np.deg2rad(twist_deg)
        cos_twist = np.cos(twist_rad)
        sin_twist = np.sin(twist_rad)
        x_upper = x_local * cos_twist - zu * sin_twist
        x_lower = x_local * cos_twist - zl * sin_twist
        z_upper = x_local * sin_twist + zu * cos_twist
        z_lower = x_local * sin_twist + zl * cos_twist
        x_min = min(float(np.min(x_upper)), float(np.min(x_lower)))
        x_upper = x_upper - x_min
        x_lower = x_lower - x_min
        raw_profiles.append((x_upper, x_lower, z_upper, z_lower, float(yy), float(chord), twist_deg))
        thickness_ranges.append(float(np.max(z_upper) - np.min(z_lower)))
    ...
    for (x_upper, x_lower, z_upper, z_lower, yy, chord, twist_deg), off, color, label in zip(
        raw_profiles, offsets, color_cycle, section_labels
    ):
        z_upper_s = z_upper + off
        z_lower_s = z_lower + off
        order_upper = np.argsort(x_upper)
        order_lower = np.argsort(x_lower)
        ax.fill(
            np.concatenate([x_upper[order_upper], x_lower[order_lower][::-1]]),
            np.concatenate([z_upper_s[order_upper], z_lower_s[order_lower][::-1]]),
            color=color,
            alpha=0.15,
            zorder=1,
        )
        ax.plot(x_upper[order_upper], z_upper_s[order_upper], color=color, linewidth=2.0, zorder=2)
        ax.plot(x_lower[order_lower], z_lower_s[order_lower], color=color, linewidth=2.0, zorder=2)
        x_right_here = max(float(np.max(x_upper)), float(np.max(x_lower)))
        ax.text(
            x_right_here + 0.9,
            off,
            f"{label}: y={yy:.2f} m | c={chord:.2f} m | twist={twist_deg:.2f} deg",
            fontsize=8.3,
            color="#1f2937",
            va="center",
        ...
        ax.plot([0.0, x_right_here], [off, off], color="#94a3b8", linewidth=0.8, linestyle=":", zorder=0)

    ax.text(
        0.6,
        offsets[0] + 0.55 * thickness_ranges[0],
        "Profiles at true scale with twist applied (stacked only for readability, no normalization)",
        fontsize=8.6,
        color="#334155",
        va="top",
        bbox={"boxstyle": "round,pad=0.18", "facecolor": "white", "edgecolor": "#cbd5e1", "alpha": 0.92},
    )
    ax.set_title("Section profiles (true scale, with twist)")
    ax.set_xlabel("local x after twist [m]")
    ax.set_ylabel("local vertical [m]")
    ax.set_aspect("equal", adjustable="box")
    ax.grid(True, linewidth=0.35, alpha=0.25)
    ax.set_yticks([])

    x_right = max(max(float(np.max(item[0])), float(np.max(item[1]))) for item in raw_profiles)
    x_left = min(min(float(np.min(item[0])), float(np.min(item[1]))) for item in raw_profiles)
    z_top = max(float(np.max(item[2] + off)) for item, off in zip(raw_profiles, offsets))
    z_bot = min(float(np.min(item[3] + off)) for item, off in zip(raw_profiles, offsets))
...
def draw_3d(
    ...
    *,
    ...
    x_le = np.zeros(ns, dtype=float)
    y_le = np.zeros(ns, dtype=float)
    z_le = np.zeros(ns, dtype=float)
    x_te = np.zeros(ns, dtype=float)
    y_te = np.zeros(ns, dtype=float)
    z_te = np.zeros(ns, dtype=float)
    ...
    for i, (yy, le, chord, twist, v0) in enumerate(
        ...
        y_le[i] = yu[i, 0]
        z_le[i] = zu[i, 0]
        x_te[i] = 0.5 * (xu[i, -1] + xl[i, -1])
        y_te[i] = float(yy)
        z_te[i] = 0.5 * (zu[i, -1] + zl[i, -1])
    ...
    ax.set_zlabel("vertical [m]", labelpad=5.0)
    ax.set_title("CTA 3D view", pad=8.0)
    ...
    try:
        ax.set_box_aspect((x_max - x_min, 1.70 * y_max, 1.45 * max(1e-9, z_max - z_min)))
    except Exception:
        pass
    ax.grid(False)
    for axis in (ax.xaxis, ax.yaxis, ax.zaxis):
        try:
            ...
            pass
    ax.tick_params(pad=1.5, labelsize=9.0)
...
def main() -> None:
    ...
    fixed_values = cta_fixed_values(cta_design=design)
    cta_view = build_cta_design_space(cta_design=design).cta_flat()
    ...
    for stale_svg in (
        output_dir / "cta_layout.svg",
        output_dir / "cta_front.svg",
        output_dir / "cta_profiles.svg",
    ):
        if stale_svg.exists():
            stale_svg.unlink()
    ...
    ax_l_plan.set_title("CTA planform (top view, half-wing)")
    ...
    ax_front.set_title("CTA front view with span measures")
    ...
    ax_scheme_plan.set_title("CTA geometric definition scheme")
    save_figure(fig_scheme, scheme_png, dpi=230, bbox_inches="tight")
    plt.close(fig_scheme)

    fig_3d = plt.figure(figsize=(13.6, 7.1), constrained_layout=False)
    ax_3d = fig_3d.add_subplot(111, projection="3d")
    ax_3d.set_position([0.01, 0.04, 0.98, 0.92])
    draw_3d(ax_3d, prepared, dense_span, leading_edge_x, chord_dense)
    save_figure(fig_3d, view3d_png, dpi=240, bbox_inches="tight")
    plt.close(fig_3d)

    for obsolete in (
        output_dir / "cta_planform_cargo.png",
        output_dir / "cta_helper_scheme.png",
        output_dir / "cta_planform_cargo.svg",
    ):
        if obsolete.exists():
            obsolete.unlink()
