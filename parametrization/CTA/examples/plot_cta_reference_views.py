...
def draw_front(
    ...
    for x_value in (-b2_end, -b1, b1, b2_end):
    ...
    for idx, (name, x_value) in enumerate(station_labels):
        y_up = float(np.interp(abs(float(x_value)), dense_span, upper))
        ax.scatter([x_value], [y_up], color="#0f4c5c", s=18, zorder=5)
        ax.text(
            x_value + (0.38 if idx > 0 else 0.25),
            y_up + 0.18,
            name,
            fontsize=8.8,
            color="#334155",
    ...
    z_margin = 0.26 * z_span
    y_bracket_0 = float(np.min(lower)) - 0.42 * z_span
    ...
    ax.set_ylim(y_bracket_0 - 0.45, float(np.max(upper)) + z_margin)
